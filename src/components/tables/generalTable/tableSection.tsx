...
const TableSection = ({
  ...
  const handleSelect = (row: ITableBody, isSelected: boolean) => {
    ...
    onSelect && onSelect(isSelected ? removed : included);
    setSelectedRows(isSelected ? removed : included);
  };

  //   Invoke when all rows are selected
  const handleSelectAll = () => {
    const included = [...new Set([...selectedRows, ...tableBody.map((row) => row.rowId)])];
      ...
      <Table.Head>
        {selectOn && (
          <Table.HeadCell className="sticky left-0 bg-gray-50">
            <Checkbox className="cursor-pointer" onChange={handleSelectAll} checked={allSelected} />
          </Table.HeadCell>
        )}
        {tableHeaders.map((el) => (
            ...
            >
              ...
              {row.rowInfo.map((cell) => (
                <Table.Cell
                  ...
                  {...cell?.cellProps}
                  onClick={(e) => {
                    !selectOn && cell?.handleClick && cell.handleClick(e, row.rowId, cell.text);
                  }}
